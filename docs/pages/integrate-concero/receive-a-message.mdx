...
On the **destination chain**, your contract receives Concero messages through `conceroReceive(...)`, which is called by the **ConceroRouter** after the relayer submits the message and verifier checks are computed.


Concero provides two base contracts to make this safe and consistent:


* **`ConceroClientBase`**: minimal base that enforces *router-only calls*, *allowed relayer libs*, and lets you define your own verification assertion policy.

* **`ConceroClient`**: an opinionated base that enforces a *strict consensus policy* (all required verifier checks must be true + allowlist).
...
:::note
Current contract interface names still use `validationChecks` and `validatorLibs`. In this guide we refer to them as verification checks and verifier libraries.
:::
...
### 3) Your verification assertion policy must pass
...
## Choosing a verification policy
...
This is the safest default if you want a simple rule: *exactly N verifiers, all must assert*.

### Option B — Implement your own policy in `ConceroClientBase`

For example:

* “2 of 3 verifiers must assert”

* “verifier A required, plus at least one of B,C”

* “accept if ANY verifier asserts” (not recommended, but possible)
...
* requires strict verifier consensus (via `ConceroClient`),
