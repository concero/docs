...
contract YourDapp is ConceroClient {
    ...
    function sendMessage(
        ...
        address[] calldata verifierLibs,
        bytes[] calldata verifierConfigs,      // can be empty
        ...
        IConceroRouter.MessageRequest memory req = IConceroRouter.MessageRequest({
            ...
            validatorLibs: verifierLibs,           // interface naming keeps `validator*`
            validatorConfigs: verifierConfigs,     // interface naming keeps `validator*`
            relayerConfig: relayerConfig,
            dstChainData: dstChainData,
            payload: bytes(text)
        });

        // 3) Quote total fee (relayer + verifiers), then send
...
Your request is the single source of truth for pricing, receipt packing, relayer selection, and verifier setup.
...
* **+ verifier fees** (computed by verifier libs)
